...
# this function actually is useless, as that's the same to the total number difference
# TODO: refer to this code,
# calculate for KDE score
def calculate_points_quadrat_diff(target_xy, affined_source_xy, nx=3, ny=3):
    qstat_targ = qs.QStatistic(target_xy, shape="rectangle", nx=nx, ny=ny)
    qstat_aff = qs.QStatistic(affined_source_xy, shape="rectangle",  nx=nx, ny=ny)

    targ_qua_cnt_dict = qstat_targ.mr.point_location_sta()
    aff_qua_cnt_dict = qstat_aff.mr.point_location_sta()
    # targ_qua_cnt_arr = np.array(list(targ_qua_cnt_dict.values())).reshape(nx, ny)
    # aff_qua_cnt_arr = np.array(list(aff_qua_cnt_dict.values())).reshape(nx, ny)
    diff_arr = np.abs(np.array(list(targ_qua_cnt_dict.values())) - np.array(list(aff_qua_cnt_dict.values())))
...
if __name__ == "__main__":
    ...
    ROI_list = natsort.natsorted(get_TMA_core_list(MxIF_img_data_dir))
    ...
    for roi_id in ROI_list:
        ...
        if not os.path.exists(HE_quant_fn) or not os.path.exists(MxIF_quant_fn):
            ...
            quadrat_diff = calculate_points_quadrat_diff(target, r_points)
