...
def cell_detections = hierarchy.getFlattenedObjectList(null).findAll {it.isDetection()}
//def detections = hierarchy.getFlattenedObjectList(null).findAll {it.isDetection()}
//def cell_detections = QP.getCellObjects()
for (anno in annotations){
    if(anno.getPathClass().toString()){
        //TODO: get relevant cells in anno
//        saveAllCellMask(pathOutput, server, anno, cell_detections, downsample, imageExportType, false, class_txt_list, class_color_list) //save cell (nucleus+plasma)
...
//        def shape = PathROIToolsAwt.getShape(cell_roi) //if can't import PathROIToolsAwt, try the next line
        ...
        if (!pathCellObject.getPathClass()) {
